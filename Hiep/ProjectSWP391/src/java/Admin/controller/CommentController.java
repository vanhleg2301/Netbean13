...
public class CommentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("displayAll")) {
            ArrayList<Comment> list = dao.pagingComment(pageNumber, pageSize);
            request.setAttribute("data", list);
            request.setAttribute("currentPage", pageNumber);
            request.setAttribute("totalPages", dao.getTotalPages(pageSize));
            dispath(request, response, "Admin/comment.jsp");
        }

        if (service.equals("deleteComment")) {
            int id = Integer.parseInt(request.getParameter("id"));
            int n = dao.deleteComment(id);
            if (n <= 0) {
                ArrayList<Comment> list = dao.getAllComment();
                String ms = "Cannot delete Comment";
        ...
        if (service.equals("searchComment")) {
            ...
            ArrayList<Comment> list = dao.searchComment(content);
            if (list.isEmpty()) {
